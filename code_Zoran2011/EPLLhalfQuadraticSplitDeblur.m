function [cleanI,cost] = EPLLhalfQuadraticSplitDeblur(noiseI,lambda,K,patchSize,betas,T,prior,LogLFunc)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
for betaa=betas
    ...
    for tt=1:T
        ...
        % x step
        [I1,counts] = scol2im(cleanZ,patchSize,size(noiseI,1),size(noiseI,2),'sum');
        ...
        % the paper)
        tmp = bicg(@(x,tflag) Afun(x,K,counts,beta,lambda,size(noiseI),tflag),(lambda*tt1(:) + beta*tt2(:)),1e-6,2500);
        cleanI = reshape(tmp,size(noiseI));
        
        subplot(1,2,1); imshow(noiseI,[]);title('Blurred image');
        subplot(1,2,2); imshow(cleanI,[]);title('Clean image');
        drawnow;
